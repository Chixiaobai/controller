  ...
  std::ostream & out)
{
  ...
  {
    if (msg.reference_joint_angles.size() == 0) {
      out << "reference_joint_angles: []";
    } else {
      out << "reference_joint_angles: [";
      size_t pending_items = msg.reference_joint_angles.size();
      for (auto item : msg.reference_joint_angles) {
